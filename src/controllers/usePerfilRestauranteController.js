...
export const usePerfilRestauranteController = () => {
  ...
  useEffect(() => {
    const carregarDados = async () => {
      ...
      try {
        ...
        if (d) {
          if (d.imagens?.logoUrl) {
            setLogo(d.imagens.logoUrl);
            setLogoOriginal(d.imagens.logoUrl);
          }
          if (d.imagens?.capaUrl) {
            setCapa(d.imagens.capaUrl);
            setCapaOriginal(d.imagens.capaUrl);
          }
          ...
          if (d.pagamentos)
            setPagamentos((prev) => ({ ...prev, ...d.pagamentos }));
  ...
  const handleBuscarCep = async (cepLimpo) => {
    ...
    if (resultado.erro) {
      ...
      const coords = await PerfilRestauranteModel.buscarCoordenadas(
  ...
  const handleHoraChange = (dia, campo, valor) => {
    ...
    if (digitos.length >= 1) {
      const hh = parseInt(digitos.slice(0, Math.min(2, digitos.length)), 10);
      if (digitos.length >= 2 && hh > 23) {
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      if (logo !== logoOriginal) {
        urlLogo = await PerfilRestauranteModel.uploadParaCloudinary(
          logo,
          plataforma,
        );
      ...
      if (capa && capa !== capaOriginal) {
        urlCapa = await PerfilRestauranteModel.uploadParaCloudinary(
          capa,
          plataforma,
        );
      }
      ...
      await PerfilRestauranteModel.atualizarPerfil(
        usuario.uid,
        dadosAtualizados,
      );
